...
import { useBingRunner } from '../hooks/useBingRunner';

export default function BingTool() {
  const { projects } = useProjects();
  const { state, logs, start, cancel } = useBingRunner();
  ...
  return (
    <div style={{ padding: 'var(--space-md)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 'var(--space-md)' }}>
        <h2 style={{ fontSize: 17 }}>Bing 批量提交</h2>
